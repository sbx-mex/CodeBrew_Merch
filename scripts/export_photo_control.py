...
HEADERS = ("ESTADO FOTO", "CÓDIGO DÍA", "SKU INTL", "ARTÍCULO", "HOMOLOGADO", "PRIORIDAD", "FOTO", "LOTE", "COINCIDENCIA")
GREEN = "006241"
DARK_GREEN = "073F2F"
LIGHT_GREEN = "DCF3E8"
LIGHT_RED = "FFE6E2"
...
def control_rows(payload: dict) -> list[list[object]]:
    ...
    rows.sort(key=lambda row: (row[4] != "SÍ", row[5] != "ACTIVO", row[0] == "CON FOTO", numeric_code(row[1])))
    return rows
...
def write_xlsx(path: Path, rows: list[list[object]]) -> None:
    ...
    sheet["A1"] = "CONTROL DE FOTOS · CÓDIGO DÍA"
    ...
    last_row = len(rows) + 7
    sheet["A4"] = f"=COUNTA(B8:B{last_row})"
    ...
    sheet["A6"] = "Agrega una foto nombrada sólo con Código Día o SKU internacional. El proyecto llena lote-01 antes de usar el siguiente."
    ...
    if rows:
        table = Table(displayName="ControlFotosCodigoDia", ref=f"A7:I{last_row}")
    ...
    widths = {"A": 15, "B": 13, "C": 18, "D": 34, "E": 13, "F": 13, "G": 32, "H": 12, "I": 15}
    for column, width in widths.items():
        sheet.column_dimensions[column].width = width
    sheet.freeze_panes = "A8"
    sheet.auto_filter.ref = f"A7:I{last_row}"
