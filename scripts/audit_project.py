...
def audit(root: Path) -> dict:
    ...
    workflow_quality = (root / ".github/workflows/project-quality.yml").read_text(encoding="utf-8")
    ...
    workflow_ok = (
        all(token in workflow_update + workflow_cleanup + workflow_quality for token in ("actions/checkout@v5", "actions/setup-python@v6"))
        ...
        and "pull_request:" in workflow_quality
        and "contents: read" in workflow_quality
        and "PYTHONDONTWRITEBYTECODE" in workflow_cleanup + workflow_quality
        and "scripts/cleanup_obsolete.py --report" in workflow_quality
        and "unittest discover" in workflow_quality
    )
