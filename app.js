...
$('catalogVisualCodes').innerHTML=`<div><small>Código SAP</small><b>${escapeHtml(item.idWoe||'Sin cruce')}</b></div><div><small>Código Día</small><b>${escapeHtml(item.codigoDia||product.codigoDia||'—')}</b></div><div><small>SKU POS</small><b>${escapeHtml(product.skuPos||'—')}</b></div><div><small>Calidad visual</small><b>${escapeHtml(visualQualityLabel(product))}</b></div>`;
