...
function catalogNames(item){const p=visualProductFor(item)||{},inventory=p.nombreInventario||p.nombrePos||p.displayName||'',sci=p.descripcionSci||item?.descripcionSap||'',first=catalogNameMode==='sci'?sci||inventory:inventory||sci,second=catalogNameMode==='sci'?inventory||sci:sci||inventory,code=p.skuPos?`SKU POS ${p.skuPos}`:p.skuIntl&&p.skuIntl!=='NA'?`SKU INTL ${p.skuIntl}`:`Código Día ${item?.codigoDia||p.codigoDia||'—'}`;return [first||'Artículo',second&&second!==first?second:code,code];}
...
const product=item.visualProduct,key=catalogItemKey(item),hasPhoto=Boolean(product.visual?.src),[primaryName,secondaryName]=catalogNames(item);catalogItemByKey.set(key,item);
...
<b>Día ${escapeHtml(item.codigoDia)}</b><span>${escapeHtml(catalogNames(item)[0])}</span><small>${escapeHtml(catalogNames(item)[2])}</small>
...
$('catalogNameToggle').textContent=catalogNameMode==='sci'?'Nombre: Descripción SCI':'Nombre: Inventario';$('catalogNameToggle').addEventListener('click',event=>{catalogNameMode=catalogNameMode==='sci'?'inventory':'sci';event.currentTarget.textContent=catalogNameMode==='sci'?'Nombre: Descripción SCI':'Nombre: Inventario';persistCatalogState();renderCatalog($('woeSearch').value);});
