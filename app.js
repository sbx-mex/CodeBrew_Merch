...
const unique=[];const seen=new Set();for(const row of matches){const visualKey=row.item.visualProduct.visual?.src||row.item.visualProduct.articleKey;if(seen.has(visualKey))continue;seen.add(visualKey);unique.push(row.item);}unique.sort((a,b)=>Number(b.visualProduct.stockPriority==='active')-Number(a.visualProduct.stockPriority==='active')||Number(Boolean(b.visualProduct.visual?.src))-Number(Boolean(a.visualProduct.visual?.src))||Number(operationalWoeStatus(b).kind==='ok')-Number(operationalWoeStatus(a).kind==='ok')||articleName(a).localeCompare(articleName(b),'es'));
