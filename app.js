...
let catalogYear='all',catalogCampaign='all',catalogSort='name';
...
function visualStyle(product){const src=String(product?.visual?.src||'').replace(/[^a-zA-Z0-9_./-]/g,''),revision=String(product?.visual?.revision||'').replace(/[^a-fA-F0-9]/g,'').slice(0,16),url=revision?`${src}?v=${revision}`:src;return url?`--catalog-image:url(${url});--catalog-size:contain;--catalog-position:center`:'';}
function catalogImageUrl(product){const src=String(product?.visual?.src||'').replace(/[^a-zA-Z0-9_./-]/g,''),revision=String(product?.visual?.revision||'').replace(/[^a-fA-F0-9]/g,'').slice(0,16);return src?(revision?`${src}?v=${revision}`:src):'';}
function catalogBatchSize(){return window.matchMedia('(max-width:540px)').matches?6:window.matchMedia('(max-width:1150px)').matches?8:12;}
function persistCatalogState(){try{sessionStorage.setItem('codebrew-catalog-state-v3',JSON.stringify({year:catalogYear,campaign:catalogCampaign,type:catalogCategory,sort:catalogSort,name:catalogNameMode}));}catch(error){}}
function restoreCatalogState(){try{const saved=JSON.parse(sessionStorage.getItem('codebrew-catalog-state-v3')||'{}');catalogYear=String(saved.year||'all');catalogCampaign=String(saved.campaign||'all');catalogCategory=String(saved.type||'all');catalogSort=['stock','name','day'].includes(saved.sort)?saved.sort:'name';catalogNameMode=saved.name==='sci'?'sci':'inventory';}catch(error){}}
...
function catalogNameSources(item){const p=visualProductFor(item)||{},inventory=p.microsNames?.[0]||(item?.micros||[])[0]||p.nombreInventario||p.nombrePos||p.displayName||'',sci=p.sapDescriptions?.[0]||item?.descripcionSap||p.descripcionSci||'',code=p.skuPos?`SKU POS ${p.skuPos}`:p.skuIntl&&p.skuIntl!=='NA'?`SKU INTL ${p.skuIntl}`:`Código Día ${item?.codigoDia||p.codigoDia||'—'}`;return {inventory,sci,code};}
function catalogNames(item){const names=catalogNameSources(item),selected=catalogNameMode==='sci'?names.sci||names.inventory:names.inventory||names.sci;return [selected||'Artículo',names.code];}
function updateCatalogNameToggle(){const button=$('catalogNameToggle');if(!button)return;const showingSci=catalogNameMode==='sci';button.textContent=showingSci?'Mostrar Nombre Inventario':'Mostrar Descripción SCI';button.setAttribute('aria-label',`Cambiar a ${showingSci?'Nombre Inventario':'Descripción SCI'}`);button.setAttribute('aria-pressed',String(showingSci));}
...
function syncCatalogSelect(id,values,current,allLabel){const select=$(id);if(!select)return 'all';const clean=[...new Set(values.filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'es',{numeric:true}));const next=current==='all'||clean.includes(current)?current:'all';select.innerHTML=`<option value="all">${allLabel}</option>`+clean.map(value=>`<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`).join('');select.value=next;return next;}
...
const query=normalizeText(raw),tokens=query.split(' ').filter(Boolean),compactQuery=query.replace(/\s+/g,''),signature=`${appMode}|${catalogYear}|${catalogCampaign}|${catalogCategory}|${catalogSort}|${microsCount}|${query}`;
...
const fullCatalogPool=allMatches.filter(row=>row.item.visualProduct&&(!merchMode||isMerchProduct(row.item.visualProduct))),photoPool=fullCatalogPool.filter(row=>Boolean(row.item.visualProduct.visual?.src));
const facetRows=photoPool.map(row=>row.item.visualProduct);
catalogYear=syncCatalogSelect('catalogYear',facetRows.map(product=>product.catalogYear),catalogYear,'Todos los años');
const yearRows=facetRows.filter(product=>catalogYear==='all'||product.catalogYear===catalogYear);
catalogCampaign=syncCatalogSelect('catalogCampaign',yearRows.map(product=>product.catalogCampaign),catalogCampaign,'Todas las campañas');
const campaignRows=yearRows.filter(product=>catalogCampaign==='all'||product.catalogCampaign===catalogCampaign);
const categoryValues=[...new Set(campaignRows.map(product=>product.category))];
if(catalogCategory!=='all'&&!categoryValues.includes(catalogCategory))catalogCategory='all';
$('catalogType').innerHTML='<option value="all">Todo tipo de Merch</option>'+categoryValues.sort((a,b)=>catalogCategoryLabel(a).localeCompare(catalogCategoryLabel(b),'es')).map(value=>`<option value="${escapeHtml(value)}">${escapeHtml(catalogCategoryLabel(value))}</option>`).join('');$('catalogType').value=catalogCategory;
const catalogPool=query?fullCatalogPool:photoPool.filter(row=>(catalogYear==='all'||row.item.visualProduct.catalogYear===catalogYear)&&(catalogCampaign==='all'||row.item.visualProduct.catalogCampaign===catalogCampaign)&&(catalogCategory==='all'||row.item.visualProduct.category===catalogCategory));
...
const matches=catalogPool;
const unique=[];const seen=new Set();for(const row of matches){const visualKey=row.item.visualProduct.visual?.src||row.item.visualProduct.articleKey;if(seen.has(visualKey))continue;seen.add(visualKey);unique.push(row.item);}const catalogPriority=item=>Number(Boolean(item.visualProduct?.visual?.src))*100000000+Number(item.visualProduct?.stockPriority==='active')*10000000+(Number(item.visualProduct?.stockQuantity)||0)*100;unique.sort((a,b)=>catalogPriority(b)-catalogPriority(a)||Number(operationalWoeStatus(b).kind==='ok')-Number(operationalWoeStatus(a).kind==='ok')||articleName(a).localeCompare(articleName(b),'es'));
...
const visible=unique.slice(0,catalogVisibleLimit),loadMore=$('catalogLoadMore'),remaining=Math.max(0,unique.length-visible.length);$('catalogSummary').textContent=query?`${visible.length.toLocaleString('es-MX')} de ${unique.length.toLocaleString('es-MX')} coincidencias`:`${unique.length.toLocaleString('es-MX')} fotografías disponibles`;
loadMore.hidden=!remaining;loadMore.textContent=`Ver ${Math.min(catalogBatchSize(),remaining)} más`;
$('catalogActiveFilters').innerHTML=[query?`Búsqueda global: “${escapeHtml(raw.trim())}”`:'',!query&&catalogYear!=='all'?`Año ${catalogYear}`:'',!query&&catalogCampaign!=='all'?catalogCampaign:'',!query&&catalogCategory!=='all'?catalogCategoryLabel(catalogCategory):'',`Orden: ${{stock:'existencia',name:'Nombre Inventario',day:'Código Día'}[catalogSort]}`].filter(Boolean).map(value=>`<span>${value}</span>`).join('');
grid.innerHTML=visible.length?visible.map(item=>{
const product=item.visualProduct,key=catalogItemKey(item),hasPhoto=Boolean(product.visual?.src),[primaryName]=catalogNames(item);catalogItemByKey.set(key,item);
...
return `<article class="catalog-card ${product.visualSource==='premium-override'?'is-featured':''} ${hasPhoto?'has-photo':'needs-photo'}"><div class="catalog-thumb">${hasPhoto?`<img src="${catalogImageUrl(product)}" alt="${escapeHtml(primaryName)}" loading="lazy" decoding="async" width="960" height="960" data-catalog-image>`:photoState}</div><div class="catalog-card-body"><h3 title="${escapeHtml(primaryName)}">${escapeHtml(primaryName)}</h3><div class="catalog-codes"><div><small>SAP</small><b>${escapeHtml(item.idWoe||'Pendiente')}</b></div><div><small>Día</small><b>${escapeHtml(item.codigoDia||'—')}</b></div></div>${photoRequest}<div class="catalog-card-action"><div class="catalog-quantity"><button type="button" data-catalog-step="-1" data-catalog-key="${escapeHtml(key)}" aria-label="Restar pieza">−</button><input type="number" min="1" max="9999" value="1" data-catalog-qty="${escapeHtml(key)}" aria-label="Piezas de ${escapeHtml(primaryName)}"><button type="button" data-catalog-step="1" data-catalog-key="${escapeHtml(key)}" aria-label="Agregar pieza">+</button></div><button type="button" class="catalog-add" data-catalog-add="${escapeHtml(key)}">Añadir al conteo</button></div></div></article>`;
}).join(''):'<div class="catalog-empty"><b>No encontramos artículos.</b><br>Prueba otra palabra, Código Día, SAP o categoría.</div>';
grid.querySelectorAll('[data-catalog-image]').forEach(image=>image.addEventListener('error',()=>{
const card=image.closest('.catalog-card'),item=catalogItemByKey.get(card?.querySelector('[data-catalog-add]')?.dataset.catalogAdd||'');
card?.classList.remove('has-photo');card?.classList.add('needs-photo');
image.parentElement.innerHTML='<div class="catalog-missing-visual"><strong>Imagen no disponible</strong><small>La ficha del artículo sigue visible</small></div>';
if(item&&!card.querySelector('.catalog-photo-request'))card.querySelector('.catalog-codes')?.insertAdjacentHTML('afterend',`<aside class="catalog-photo-request"><a href="${missingPhotoWhatsappUrl(item)}" data-photo-whatsapp="${missingPhotoWhatsappUrl(item,true)}" rel="noopener noreferrer"><span><b>Enviar foto por WhatsApp</b><small>Ayuda a completar el catálogo</small></span></a></aside>`);
},{once:true}));
grid.querySelectorAll('[data-photo-whatsapp]').forEach(link=>link.addEventListener('click',event=>{if(!window.confirm('Este artículo no tiene fotografía. ¿Deseas abrir WhatsApp para enviarla?')){event.preventDefault();return;}if(!/Android|iPhone|iPad|iPod/i.test(navigator.userAgent))return;event.preventDefault();window.location.assign(link.dataset.photoWhatsapp);}));
...
let score=Number(Boolean(row.item.visualProduct?.visual?.src))*10000000+Number(row.item.visualProduct?.stockPriority==='active')*1000000+(Number(row.item.visualProduct?.stockQuantity)||0)*10+(row.item.idWoe?12:0)+tokens.reduce((sum,token)=>sum+(sap.startsWith(token)?8:sap.includes(token)?5:micros.includes(token)?4:2),0);
...
<b>Día ${escapeHtml(item.codigoDia)}</b><span>${escapeHtml(catalogNames(item)[0])}</span><small>${escapeHtml(catalogNames(item)[1])}</small>
...
function htmlInputCell(row,column){
const cell=row.querySelector(`.mat-column-${column},.cdk-column-${column}`),input=cell?.querySelector('input');
return String(input?.value||input?.getAttribute('value')||input?.getAttribute('ng-reflect-model')||cell?.textContent||'').replace(/\s+/g,' ').trim();
}
function isSapPrecountHtml(document){
return Boolean(document.querySelector('.mat-column-lcodSAP,.cdk-column-lcodSAP')&&document.querySelector('.mat-column-sdescripcion,.cdk-column-sdescripcion')&&document.querySelector('.mat-column-sunidadMedidaBase,.cdk-column-sunidadMedidaBase'));
}
...
const sourceSap=normalizeSku(htmlCell(row,preCount?'lcodSAP':'codSap')),sourceDescription=htmlCell(row,preCount?'sdescripcion':'description'),sourceFamily=preCount?'':htmlCell(row,'family'),rawQty=preCount?htmlInputCell(row,'amount'):htmlCell(row,'amount'),parsedQty=parseStockNumber(rawQty),qty=preCount&&parsedQty===null?0:parsedQty,pdfUnit=htmlCell(row,preCount?'sunidadMedidaBase':'UMB')||'N/D',stockValue=parseStockNumber(htmlCell(row,preCount?'precio':'valorStock'));
...
return stockMeta?.preCount||$('stockIncludeZero')?.checked?stockSourceRows:stockSourceRows.filter(row=>Math.abs(row.qty)>tolerance||(row.stockValue!==null&&Math.abs(row.stockValue)>tolerance));
}
...
if(previous&&previous!==next){microsCount='all';catalogRenderSignature='';const c=$('microsCountFilter');if(c)c.value='all';}
...
renderCatalog('');
$('catalogLoadMore').addEventListener('click',()=>{catalogVisibleLimit+=catalogBatchSize();renderCatalog($('woeSearch').value);});
$('catalogYear').addEventListener('change',event=>{catalogYear=event.target.value;catalogCampaign='all';catalogCategory='all';catalogRenderSignature='';persistCatalogState();renderCatalog($('woeSearch').value);});
$('catalogCampaign').addEventListener('change',event=>{catalogCampaign=event.target.value;catalogCategory='all';catalogRenderSignature='';persistCatalogState();renderCatalog($('woeSearch').value);});
$('catalogType').addEventListener('change',event=>{catalogCategory=event.target.value;catalogRenderSignature='';persistCatalogState();renderCatalog($('woeSearch').value);});
$('catalogSort').value=catalogSort;$('catalogSort').addEventListener('change',event=>{catalogSort=event.target.value;persistCatalogState();renderCatalog($('woeSearch').value);});
updateCatalogNameToggle();$('catalogNameToggle').addEventListener('click',()=>{catalogNameMode=catalogNameMode==='sci'?'inventory':'sci';updateCatalogNameToggle();persistCatalogState();renderCatalog($('woeSearch').value);});
$('catalogReset').addEventListener('click',()=>{catalogYear='all';catalogCampaign='all';catalogCategory='all';catalogSort='name';$('catalogSort').value='name';$('woeSearch').value='';$('woeSearchClear').hidden=true;catalogRenderSignature='';persistCatalogState();renderCatalog('');$('woeSearch').focus();});
